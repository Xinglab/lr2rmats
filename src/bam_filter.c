...
int remove_overlap(bam1_t *b, read_trans_t *r)
{
...
int gtf_filter(bam1_t *b, int *score, int *intron_n, float cov_rate, float map_qual, read_trans_t *r)
{
    ...
    *intron_n=0;
    ...
    if (remove_overlap(b, r)) return 1;
...
{
    int c; float cov_rat=COV_RATIO, map_qual = MAP_QUAL, sec_rat=SEC_RATIO; int min_intron_n = MIN_INTRON_NUM; char remove_fn[1024]="";
    int cnt=0;
    while ((c = getopt_long(argc, argv, "v:q:s:i:r:", filter_long_opt, NULL)) >= 0) {
        switch (c) {
